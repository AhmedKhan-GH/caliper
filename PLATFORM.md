...
### Phase 5 — ecosystem
- `caliper-registry` repo + schema CI; Browse tab (install/update/uninstall, badges); `caliper install/publish`; docs site (mkdocs: "first applet in 10 minutes", service reference generated from headers, ABI hygiene guide); macOS codesign/notarize + simple update check; delete `applets/` from the monorepo entirely — the `hello` fixture moves to `examples/hello`.
