...
- [x] Telemetry ingestion **DECIDED + SHIPPED** (strategy call 2026-07-12): live external data becomes a platform **service**, not a per-applet pattern — `caliper.feed.v1` (host-owned providers, per-channel ring buffers with seq+t_ns, a pull-based any-thread non-blocking C read surface) + `pulse_scope`, the dashboard exemplar that freezes it (enumerates channels dynamically, ~10 Hz job-thread poller, input-locked ImPlot strip charts, the honest-loss gap counter surfaced in UI). macOS provider **live-verified** — 7 sudo-free channels @10 Hz (`sys.cpu.util`, `sys.mem.used`, `sys.thermal.state`, `sys.gpu.util`, `sys.fan.rpm`, `sys.temp.battery`, `sys.power.battery`), run-proven under CPU load. The whitepaper's local-loop claim is untouched here (restatement rides the flagship). Named follow-ups (feed spec §6): the **machine-thermal-twin flagship**, the **Windows provider** pass — **shipped 2026-07-12**, 6 admin-free channels @10 Hz live-verified on the RTX 500 Ada box (`sys.cpu.util`, `sys.mem.used`, `sys.gpu.util`/`sys.gpu.temp`/`sys.gpu.power` via runtime-loaded NVML with RTD3 skip-on-fail + enforced-limit junk gate, `sys.power.battery`; CPU temp/fan honestly absent — kernel-driver-gated), run-proven under build load and through the `embed_host` path (`553f7eb`, spec `2026-07-12-feed-v1-windows-provider-pass.md`) — applet-registered sources, the Compass telemetry pane. Spec: `docs/superpowers/specs/2026-07-12-feed-v1-telemetry-design.md`; T1 `477f9e5`, T2 `16dd30c`/`b9c1c71`, T3 this exemplar + closeout series.
...
- [ ] R4 `libcaliper` second host — platform call (PLATFORM.md Phases 3–6), not geometry's. **L1+L2 shipped** (embeddable core + embed ABI + `embed_host`, run-proven on both platforms — Metal/macOS and Vulkan/Windows); the box stays unticked because the full R4 call is Compass (L3), gated on a named workflow. Spec: `docs/superpowers/specs/2026-07-11-libcaliper-compass-design.md`.
    ...
    - [x] L2 — embed C ABI `include/caliper/embed.h` v1 + `examples/embed_host` (AppKit + Win32 hosts, five calls, no GLFW); `instance_scope`/`mesh_scope` run zero-copy under embed_host on **both platforms** — Metal @2x Retina and Vulkan/Windows WINDOW canvas (RTX 500 Ada, 2026-07-11), provenance captured; embed battery 8/8 + §7 byte-compare on Vulkan OFFSCREEN, gfx unregressed both boxes (L2a `2eae15a`; L2b `a3fd333`/`1bf24a1`/`59bcc51`; Vulkan/Windows seam `c8e6005`/`852c398`/`37d87f2`, supporting `0614d51`/`0724161`/`d7f9cc4`). Honest v0 gaps: applet `log.v1` bypasses `log_fn` to stderr, `data_dir` ignored, GL refuses embed, `WM_DPICHANGED` mixed-DPI unexercised
