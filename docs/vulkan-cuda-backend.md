...
On the RTX 500 Ada the suite is **16/16, 79 assertions**, all rungs pixel-exact. This replaces the earlier `CALIPER_VULKAN_SELFTEST` app self-test (removed).

**V4 semaphore pipelining (§8 D21) — implemented & verified 2026-07-05.** The CUDA→Vulkan handoff is now GPU-ordered via a **per-texture shared timeline semaphore** (`VK_KHR_timeline_semaphore` exported opaque-Win32 → `cuImportExternalSemaphore`), removing every CPU sync from the hot path. Per update: CUDA's stream-ordered copy (`cuMemcpyDtoDAsync`) signals `base+1`; the texture's Vulkan pass GPU-waits `base+1` and signals `base+2`; the frame submission GPU-waits `base+2` before ImGui samples (accumulated in `pending_frame_waits_`, consumed in `frame_render`). No `cuCtxSynchronize`, no fence wait — the only host wait is `retire()`, back-pressure when the *previous* update of the *same* texture is still in flight (needed for command-buffer re-record + shared-buffer overwrite safety; instant in steady state). Each pipelined texture gets its own command buffer, descriptor set, and LUT buffer so concurrent in-flight chains never share mutable state. Pairing requires the device to export timeline semaphores (`timelineSemaphore` feature + `VK_KHR_external_semaphore_win32`); where absent, per-texture creation fails closed and that texture uses the **synchronous fallback** (the prior D21 model — copy + `cuCtxSynchronize` + fenced submit), so correctness never depends on the optimization. A new `gfx-cuda` test bursts 8 back-to-back updates with no readback between them — genuinely overlapping chains ordered only by the semaphore — and asserts the final frame is byte-exact. Startup logs `sync mode: pipelined` or `sync mode: synchronous`.

**V5 imported allocations (bridge v1.2) — implemented & hardware-verified 2026-07-07.** The bridge imports an applet-exported CUDA VMM block into Vulkan once (`import_allocation`: `VkImportMemoryWin32HandleInfoKHR` + `DuplicateHandle`, full unwind on failure) and updates textures **directly from it at byte offsets** (`update_texture_from_alloc` → `tex_update_from_imported`: descriptor-offset f32 compute / `bufferOffset` u8 blit, **no D2D copy**, pipelined signal-without-copy with the fenced-sync fallback). Applet side, `caliper::adapters::ExportablePool` is a torch-2.5.1 `MemPool` over `cuMemCreate` shareable blocks with a self-contained nine-symbol driver mini-loader; `gpt_scope` opts in when the host grants `CALIPER_BRIDGE_CAP_IMPORT_ALLOC`. **No new host driver symbols in the core `Api`** — the host gained a separate *optional* `VmmApi` table (`cuda_driver.{h,cpp}`, null if any symbol is missing) used by the Task-6 tests to build shareable allocations the host doesn't otherwise create. First-compile fixes and hardware findings from the Windows box (all fixed forward): the pool header's torch/c10 includes sat inside `namespace caliper::adapters` (compiled out on mac); a WIN32-shareable `cuMemCreate` with null `win32HandleMetaData` returns `CUDA_ERROR_INVALID_VALUE` (needs `SECURITY_ATTRIBUTES`, the memMapIPCDrv precedent); `beginAllocateToPool` before torch's lazy CUDA init terminates the process (the pool ctor now forces `lazyInitCUDA`). Verified on the RTX 500 Ada: five new `gfx-cuda` rows (byte-exact at offsets 0/512 through `"compute-imported"`, misaligned-offset fallback, u8 RGBA `"blit-imported"`, release + 50× import/release id monotonicity, out-of-bounds rejection) — the full gfx suite is 22 cases / 1030 assertions green — plus a live `gpt_scope` run whose heatmap status line reads **"zero-copy (imported pool)"** with training concurrent, cancel/relaunch clean, and a `CALIPER_RENDERER=gl` run falling back byte-identically.

The `gfx-cuda` rows are hardware-gated, so a lavapipe/SwiftShader CI run covers only the pure-Vulkan matrix; the CUDA byte-equality rows run on a self-hosted NVIDIA box (as §5 always intended). **All five increments (V1–V5) are now built and verified.**
...
4. **Synchronize CUDA → Vulkan.** *Implemented as V4 semaphore pipelining* (see the status note above): a per-texture shared timeline semaphore GPU-orders the copy → colormap → frame-sample chain, so no CPU synchronize sits on the hot path. The original v1 model — `cuCtxSynchronize` before a fenced Vulkan submit (Metal's `waitUntilCompleted` shape) — remains the automatic fallback where the device does not export timeline semaphores.
...
| **V4** ✅ | Semaphore pipelining (`cudaImportExternalSemaphore`) — **done 2026-07-05** | GPU-ordered handoff via per-texture shared timeline semaphore; no CPU sync on the hot path; synchronous fallback where timeline semaphores absent; burst-overlap gfx test byte-exact |
...
| D21 | **Synchronous interop in v1** (`cudaStreamSynchronize` + fenced submits), semaphores deferred to V4 → **V4 shipped 2026-07-05**: shared timeline semaphore pipelines the handoff, synchronous path retained as fallback | Correctness-first held: the pipelined path was added only after the synchronous path was verified byte-exact, and degrades to it when timeline semaphores are unavailable |
