...
The `gfx-cuda` rows are hardware-gated, so a lavapipe/SwiftShader CI run covers only the pure-Vulkan matrix; the CUDA byte-equality rows run on a self-hosted NVIDIA box (as §5 always intended). **All four increments (V1–V4) are now built and verified.**
