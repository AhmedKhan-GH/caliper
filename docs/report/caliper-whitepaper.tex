...
% Source of truth: WHITEPAPER.md v0.1 (branch feat/zerocopy-arbitrary-cuda),
% with the appendices realized and §6.5 covering the bridge-v1.2 exportable
% pool (hardware-verified July 2026).
...
\author{Ahmed Khan}
\date{July 2026 --- draft v0.1 (\LaTeX{} edition)}
...
block. Failure modes are designed to be loud: acceptance violations log and
...
\begin{itemize}
  ...
  \item Dense outputs (heatmaps, feature maps, attention grids) are where the
        zero-copy path matters; coordinate-style graphics (point clouds,
        curves) flow through plotting libraries as small CPU arrays ---
        kilobytes, and the two routes compose freely.
