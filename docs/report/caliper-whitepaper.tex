...
% Source of truth: WHITEPAPER.md v0.4 (branch main), with the appendices
...
% July 2026. v0.4 adds the telemetry rung (caliper.feed.v1 + pulse_scope, §4/§9)
% and updates the embed core to run-proven on both ecosystems (§10).
...
\author{Ahmed Khan}
\date{2026-07-12 --- draft v0.4 (\LaTeX{} edition)}
...
\begin{itemize}
  ...
  \item \textbf{pulse\_scope --- the machine watching itself (telemetry,
        live).} The physical-twin ladder opens with an \emph{input} rung:
        \texttt{caliper.feed.v1} lets the \emph{host} own telemetry providers
        behind a pull-based C ABI --- named channels of timestamped,
        sequence-numbered samples read through caller-held cursors, with data
        loss surfaced as sequence gaps rather than hidden silently. On macOS
        the provider is live-verified: seven sudo-free channels at 10\,Hz (CPU
        and GPU utilization, memory used, thermal state, fan rpm, battery
        temperature and power). \texttt{pulse\_scope} draws them as
        input-locked strip charts of the machine's own vitals --- run-proven
        under CPU load (the CPU pinned at 100\%, the fan climbing
        $3{,}949 \rightarrow 6{,}413$~rpm in the captured data) --- and it is
        host-neutral: the same binary runs under Caliper and under the embed
        host. It is deliberately a dashboard: no model rides the feed yet
        (\S10).
...
\begin{itemize}
  ...
  \item Single-machine, in-process by design. Remote/cluster training is a
        ...
        loop.
  \item The local-loop claim, scoped honestly: the loop is local \emph{from
        ingestion onward}. When a feed carries real-world telemetry (\S4), the
        sensor's own transport latency is upstream of the loop and never
        hidden --- samples carry their acquisition timestamp, and staleness
        and loss are surfaced in the UI, not smoothed over. The current feed
        exemplar is a dashboard; no model learns from the feed yet. The Windows
        telemetry provider is likewise pending --- its capability bit stays
        unset there until a provider pass ships with artifacts, the same
        per-platform honesty the rest of the system uses.
\end{itemize}
...
applets without owning the machinery. A GLFW-free native host
(\texttt{embed\_host}) now runs the applets zero-copy on \emph{both} ecosystems
--- Metal/MPS on Apple Silicon and Vulkan into a Windows HWND (2026-07-12
Windows pass) --- so the embedding path is run-proven, no longer
platform-pending. The embed ABI's v1.1 revision turned embedders from mere
renderers into \emph{consumers}: a host can now pull the applets' own service
tables (\texttt{get\_service}) and read a metrics surface as
SQL~$\rightarrow$~Arrow, proven by Compass, an external wxWidgets host (Compass
itself is unpublished). A full sibling host remains a design-gated future, not
a claim.
...
live from the same \texttt{feed.v1} samples, its prediction drawn against the
measured truth the way TwinScope drapes a simulated field on a shape. That is
the named next step, not a claim made here.
