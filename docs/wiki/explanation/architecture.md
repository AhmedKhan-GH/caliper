...
!!! note "Renderer-agnostic by contract"
    The ABI never mentions a graphics API. Textures cross the boundary as an opaque `CaliperTextureId`, and applets render exclusively through ImGui/ImPlot. Native backends (Metal on macOS, Vulkan on Windows) are the target, with OpenGL 3.3 core as a frozen fallback — but because the contract hides all of this, backend changes require no epoch bump and no applet rebuilds. See `PLATFORM.md` §5.4.
